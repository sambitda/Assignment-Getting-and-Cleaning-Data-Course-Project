#Basic Assumption
#The R code in run_analysis. It is assumed that that the zip file available at https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip is downloaded and extracted in the R Home Directory.

#Libraries Used : 
#The libraries used in this operation are data.table and dplyr. 
I prefer data.table as it is efficient in handling large data as tables. dplyr is used to aggregate variables to create the tidy data.
...
#Part 1 - In order to Merge the training and the test sets to create one data set, I have combined the respective data in training and test data sets corresponding to subject, activity and features. The results are stored in subject, activity and features.
subject <- rbind(subjectTrain, subjectTest)
activity <- rbind(activityTrain, activityTest)
features <- rbind(featuresTrain, featuresTest)
#aming the columns
#The columns in the features data set has been named from the metadata in featureNames
...
#Firstly, I have set Subject as a factor variable.
extractedData$Subject <- as.factor(extractedData$Subject)
extractedData <- data.table(extractedData)
#Then I created tidyData as a data set with average for each activity and subject. Then, we order the enties in tidyData and write it into data file Tidy.txt that contains the processed data.
